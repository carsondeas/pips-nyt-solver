...
class SaRunner:
    ...
    def plot_mean_times(self, filename="sa_mean_times.png"):
        ...
        plt.title("Annealing mean solve time by difficulty")
        plt.tight_layout()
    ...
    def plot_success_rates(self, filename="sa_success_rates.png"):
        labels = []
        rates = []
        for diff in self.difficulties:
        ...
        plt.ylim(0, 100)
        plt.title("Annealing success rate by difficulty")
        plt.tight_layout()
        out_path = self.output_dir / filename
        plt.savefig(out_path, dpi=200)
        plt.close()
